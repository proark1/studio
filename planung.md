...
- 2026-07-02: **Konsistenz-Pass + 3 Features + Mehrsprachigkeit (DE/TR/AR) gebaut & verifiziert.** **① Zahlen-Konsistenz:** ein `crm.fact`-Objekt (10 Standorte, 2.410 Mitglieder, MRR 148,9k, Watchlist 34) als Quelle der Wahrheit; Zahlungs-KPIs jetzt franchiseweit-konsistent (27 + 5 + 2.378 bezahlt = 2.410 ✓, offener Betrag 1.612 € passt zu 27 Fällen), Tabellen explizit als „Auszug/dringendste Fälle" gelabelt (Mitglieder, Leads, Verträge), P&L auf 10-Standorte-Logik skaliert (Top-4 = 69k von 148,9k MRR, Label „4 von 10, Auszug"), Reaktionszeit-KPI konsistent zu Lead-Timern („Ø 11 Min · 2 Leads über 60-Min-Ziel"), Retention-KPI verweist auf die 34er-Watchlist. **② Neue Features:** 🎂 Geburtstags-Karte (Sara wird 11 → gratulieren + Party-Upsell, App-Home), 🎁 Referral-Karte mit persönlichem Code NICOLE-10 + Status (Konto), 📊 Trainer-Stats („Meine Retention 92 % · Puls 4,6 · Feedback 81 %" mit fairem Standort-Vergleich, Trainer-View). **③ Mehrsprachigkeit:** i18n-System (`T`-Dictionary, `t()`, localStorage-Persistenz) mit **Sprachumschalter** in Website-Nav (Select) + App-Konto (Chips); übersetzt: Nav, Hero, CTAs, Footer, App-Tabbar, Begrüßung, „Heute" — **Türkisch** komplett für Kernbereiche, **Arabisch inkl. echtem RTL** (`dir=rtl`, gespiegelte Border/Bubbles/Slash per CSS); transparenter Hinweis-Banner in TR/AR („Kernbereiche übersetzt — vollständige Übersetzung in V1, WhatsApp-Support auf TR/AR"). Assets `?v=16`; DOM-verifiziert (TR-Hero/Tabs, AR-RTL, Persistenz, Rückschalten), keine Konsolenfehler.
